...
def call_llm(system_prompt: str, user_message: str, temperature: float = 0.3) -> str:
    ...
    if resp.status_code == 200:
        ...
        # 2. Extract content between [RESULT] and [/RESULT] tags (New Delimiter Logic)
        import re as _re
        tag_match = _re.search(r"\[RESULT\](.*?)\[/RESULT\]", answer, _re.DOTALL)
        if tag_match:
            answer = tag_match.group(1).strip()
        elif "[[RESULT_START]]" in answer: # Support alternative formats if model slips
            tag_match = _re.search(r"\[\[RESULT_START\]\](.*?)\[\[RESULT_END\]\]", answer, _re.DOTALL)
            if tag_match:
                answer = tag_match.group(1).strip()

        if not answer:
            logger.warning("⚠️ LLM returned empty content after filtering — using raw response as fallback")
                ...
                try:
                    ...
                    if candidates:
                        ...
                        if parts:
                            ...
                            if content:
                                # Logic to start yielding only after [RESULT] or [[RESULT_START]]
                                if not found_start:
                                    if "[RESULT]" in content:
                                        found_start = True
                                        content = content.split("[RESULT]")[1]
                                    elif "[[RESULT_START]]" in content:
                                        found_start = True
                                        content = content.split("[[RESULT_START]]")[1]
                                
                                if found_start:
                                    # Basic cleanup for the end tag if it appears in a chunk
                                    if "[/RESULT]" in content:
                                        content = content.split("[/RESULT]")[0]
                                    elif "[[RESULT_END]]" in content:
                                        content = content.split("[[RESULT_END]]")[0]
...
- **THE FENCE RULE:** Your entire final response (including greeting, tables, and disclaimer) MUST be enclosed within **[RESULT]** and **[/RESULT]** tags.
- **NO PREAMBLE:** NEVER show your reasoning process, thinking steps, or evidence evaluation outside or inside the tags.
- **NO INTERNAL HEADERS:** Do NOT write "User Question:", "Context provided:", "Evidence:", "Reasoning:", or any meta-analysis.
- **DIRECT START:** Inside the **[RESULT]** tags, begin your response directly with the answer or a short greeting to {user_name}.
- **FINAL ANSWER ONLY:** Only output the final polished response meant for the user. 
- **NO HALLUCIDATED FOOTERS:** Do not generate your own footers or attribution lines (e.g., "Generated by...").
- **STRICT TAG USAGE:** [RESULT] Your Answer Here [/RESULT]
